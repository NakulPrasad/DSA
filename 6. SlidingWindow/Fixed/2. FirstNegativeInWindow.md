...
class Solution {
    ...
    public List<Integer> printFirstNegativeInteger(int n, int k, int[] arr) {
        ...
        while (r < n) {
            //save the -ve occured in array
            if (arr[r] < 0) {
                neg.add(arr[r]);
            }
            // When ws is less than k
            if (r - l + 1 < k) {
                r++;
            } 
            else if (r - l + 1 == k) {
                //push the first element in negative list to ans list
                if (neg.isEmpty()) {
                    ans.add(0);
                } 
                // if not present save zero to ans
                else {
                    ans.add(neg.peek());
                    if (arr[l] == neg.peek()) {
                        neg.poll(); // remove from front
                    }
                }
                //shift window
                l++;
