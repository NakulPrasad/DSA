...
You have a car with an unlimited gas tank and it costs `cost[i]` of gas to travel from the `i` station to its next `(i + 1)` station. You begin the journey with an empty tank at one of the gas stations.
...
- For each postion, count the stops car needs to take. if its equal to length then we can say it covers one rotation.
    ...
    public int canCompleteCircuit(int[] gas, int[] cost) {
        ...
        int stopcount =0;
            while(stopcount < n){
                ...
                // if petrol is negative then break
                if(surplus < 0){
                    break;
                }
                stopcount++;
            ...
            if(stopcount == n && surplus >= 0) return i;
        }
        return -1;
    }
}
