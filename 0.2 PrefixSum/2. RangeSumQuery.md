#### ✅ Question  
Design a class `NumArray` that allows querying the sum of elements in a given range `[left, right]` of an integer array efficiently, especially when multiple queries are expected.

---

### 🔍 Algorithm

1. Initialize a prefix sum array `prefix` of the same size as the input array `nums`.
2. Set `prefix[0] = nums[0]`.
3. For each index `i` from `1` to `n - 1`, calculate:  
   `prefix[i] = prefix[i - 1] + nums[i]`
4. To get the sum of elements from index `left` to `right`:  
   - If `left == 0`: return `prefix[right]`  
   - Else: return `prefix[right] - prefix[left - 1]`  
     (Subtracting prefix before `left` removes the extra sum.)

---

### 🧾 Pseudo-Code

```java
class NumArray {
    int[] prefix;

    constructor NumArray(nums):
        n ← length of nums
        prefix ← new array of size n
        prefix[0] ← nums[0]

        for i from 1 to n - 1:
            prefix[i] ← prefix[i - 1] + nums[i]

    method sumRange(left, right):
        if left == 0:
            return prefix[right]
        else:
            return prefix[right] - prefix[left - 1]
```

---

### 💡 Time & Space Complexity
Time (Preprocessing): O(n)

Time (Query): O(1)

Space: O(n) for the prefix array